...
Aincm1(1) = floor(heaviside(n))*Ao*sin(omega*(t-dt/2));
...
Hsyx(Np+2:Nx-Np,Np+1,Np+1:Nz-Np) = Hsyx(Np+2:Nx-Np,Np+1,Np+1:Nz-Np) - 1./(mux(Np+2:Nx-Np,Np+1,Np+1:Nz-Np)).*ones(length(Np+2:Nx-Np),1,length(Np+1:Nz-Np))*Aincm1(2)/dy; % Left
Hsyx(Np+2:Nx-Np,Ny-Np,Np+1:Nz-Np) = Hsyx(Np+2:Nx-Np,Ny-Np,Np+1:Nz-Np) + 1./(mux(Np+2:Nx-Np,Np+1,Np+1:Nz-Np)).*ones(length(Np+2:Nx-Np),1,length(Np+1:Nz-Np))*Aincm1(Ninc-Np)/dy;
                                                                                                                                                                        % Right
...
Ax = 1./(epEx/dt^4+(sigyEx+sigzEx)/(2*dt^3)).*(-(dHzdym1-2*dHzdym3+dHzdym5)/dt^2 - sigzEx./epEx.*(dHzdym1-dHzdym5)/(2*dt) + (dHydzm1-2*dHydzm3+dHydzm5)/dt^2 ...
     + sigyEx./epEx.*(dHydzm1-dHydzm5)/(2*dt) - epEx.*(-4*Axm1+6*Axm3-4*Axm5+Axm7)/dt^4 - (sigyEx+sigzEx).*(-Axm1+Axm5-.5*Axm7)/dt^3 ...
     - sigyEx.*sigzEx./epEx.*(Axm1-2*Axm3+Axm5)/dt^2 + epEx.*(gradfxm1-2*gradfxm3+gradfxm5)/dt^2 + (sigyEx+sigzEx).*(gradfxm1-gradfxm5)/(2*dt) ...
     + sigyEx.*sigzEx./epEx.*gradfxm3);
...
Ay = 1./(epEy/dt^4+(sigzEy+sigxEy)/(2*dt^3)).*(-(dHxdzm1-2*dHxdzm3+dHxdzm5)/dt^2 - sigxEy./epEy.*(dHxdzm1-dHxdzm5)/(2*dt) + (dHzdxm1-2*dHzdxm3+dHzdxm5)/dt^2 ...
     + sigzEy./epEy.*(dHzdxm1-dHzdxm5)/(2*dt) - epEy.*(-4*Aym1+6*Aym3-4*Aym5+Aym7)/dt^4 - (sigzEy+sigxEy).*(-Aym1+Aym5-.5*Aym7)/dt^3 ...
     - sigzEy.*sigxEy./epEy.*(Aym1-2*Aym3+Aym5)/dt^2 + epEy.*(gradfym1-2*gradfym3+gradfym5)/dt^2 + (sigzEy+sigxEy).*(gradfym1-gradfym5)/(2*dt) ...
     + sigzEy.*sigxEy./epEy.*gradfym3);
...
Az = 1./(epEz/dt^4+(sigxEz+sigyEz)/(2*dt^3)).*(-(dHydxm1-2*dHydxm3+dHydxm5)/dt^2 - sigyEz./epEz.*(dHydxm1-dHydxm5)/(2*dt) + (dHxdym1-2*dHxdym3+dHxdym5)/dt^2 ...
     + sigxEz./epEz.*(dHxdym1-dHxdym5)/(2*dt) - epEz.*(-4*Azm1+6*Azm3-4*Azm5+Azm7)/dt^4 - (sigxEz+sigyEz).*(-Azm1+Azm5-.5*Azm7)/dt^3 ...
     - sigxEz.*sigyEz./epEz.*(Azm1-2*Azm3+Azm5)/dt^2 + epEz.*(gradfzm1-2*gradfzm3+gradfzm5)/dt^2 + (sigxEz+sigyEz).*(gradfzm1-gradfzm5)/(2*dt) ...
     + sigxEz.*sigyEz./epEz.*gradfzm3);
